...
// SQL statments
	...
	$SelectAV = "SELECT a.ID, r.rname, a.DisplayName, d.dname FROM `accounts` a INNER JOIN `divisions` d ON a.`DivID` = d.`did` INNER JOIN `Rank` r ON a.`RankID` = r.`RankID` WHERE a.`cCode` = '$code' AND a.`active`='1' AND a.`UUID` = '$uuid'";
	...
	elseif($isRecord == 1)
	{
		...
		echo "OK|".$row['ID']."|".$row['rname']."|".$row['DisplayName']."|".$row['dname'];
	}
