...
<?php
	...
	{
		$MembersByDivision = "SELECT r.RankLogo, r.rname, a.DisplayName, a.username, t.tag_name, a.`UUID` FROM `accounts` a INNER JOIN `divisions` d ON a.`DivID` = d.`did` INNER JOIN `Rank` r ON a.`RankID` = r.`RankID` INNER JOIN `Titles` t ON a.`TitleID` = t.`tid` WHERE a.active = '1' AND NOT d.did='10' AND d.did = '" . $DivHeaders['did'] . "' ORDER BY r.`RankID`";
		...
		{   //Creates a loop to loop through results
			echo "<tr><td align='center'>" . $row['rname'] . "<br>" . $row['RankLogo'] . "</td><td><a href='secondlife:///app/agent/" . $row['UUID'] . "/about' style='color:#FDD017'>" . $row['DisplayName'] . "</a></td><td align='left'>" . $row['tag_name'] . "</td></tr>";
		}
		echo "</table>";
	}
	mysqli_close($db);
