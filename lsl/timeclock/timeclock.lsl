	...
	http_response(key req ,integer stat, list met, string body)
	{
		...
		else if( req == ClockReq ) //Response was from the TimeClock
		{
			if(stat == 200)
			{
				...
				llSay(0,"An unexpected error occured while attempting update time clock. Please visit https://github.com/CollectiveIndustries/UFGQ/issues to submit bug reports or checkup on known issues.\nSTAT: "+(string)stat+"\nRES: "+(string)body);
		}
	}
